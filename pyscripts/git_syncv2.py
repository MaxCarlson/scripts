...
import getpass
import configparser
from debug_utils import write_debug  # Import the debug function

# ANSI color codes (No longer used directly, but kept for potential future enhancements in debug_utils)
COLOR_RESET = "\033[0m"
COLOR_RED = "\033[91m"
COLOR_GREEN = "\033[92m"
...
def run_git_command(command, cwd=None, capture_output=True, check=True, verbose=False):
    ...
    try:
        ...
        write_debug(f"Executing command: {cmd_str}", channel="Info", condition=verbose)
        ...
        if capture_output and result.stderr:
            write_debug(f"Command stderr:\n{result.stderr.strip()}", channel="Verbose", condition=verbose) # Log stderr even if successful
        return result
    except subprocess.CalledProcessError as e:
        error_msg = f"Git command failed: {e}\nCommand: {' '.join(command)}\nReturn Code: {e.returncode}"
        if e.stderr:
            error_msg += f"\nError details:\n{e.stderr.strip()}"
        write_debug(error_msg, channel="Error")
        sys.exit(1)
    except FileNotFoundError:
        write_debug("Error: Git command not found. Please ensure Git is installed and in your PATH.", channel="Error")
        sys.exit(1)
...
def show_git_status(repo_path=".", verbose=False):
    ...
    write_debug("Git Status:", channel="Info", condition=True) # Always show status header
    for line in status_lines:
        if line.startswith("M"):
            write_debug(line, channel="Warning")  # Modified (Yellow in original)
        elif line.startswith("A"):
            write_debug(line, channel="Success")   # Added (Green in original)
        elif line.startswith("??"):
            write_debug(line, channel="Error")     # Untracked (Red in original)
        else:
            write_debug(line, channel="Debug", condition=verbose) # Less important status lines in verbose mode
...
def git_add_changes(add_pattern=".", repo_path=".", verbose=False):
    ...
    write_debug(f"Adding files: {add_pattern}", channel="Info", condition=True) # Always show add info
...
def show_staged_files(repo_path=".", verbose=False):
    ...
    write_debug("Staged Files:", channel="Info", condition=True) # Always show staged files header
    for line in staged_files:
        if line.startswith("M"):
            ...
            write_debug(line, channel="Info")    # Renamed (Cyan in original - Info level is fine)
        elif line.startswith("C"):
            write_debug(line, channel="Info")    # Copied (Cyan in original - Info level is fine)
        elif line.startswith("U"):
            write_debug(line, channel="Warning")  # Unmerged (Yellow - needs resolving)
        else:
            write_debug(line, channel="Debug", condition=verbose) # Less important staged file info in verbose mode
...
def git_commit_changes(commit_message, repo_path=".", verbose=False):
    ...
    write_debug("Committing changes", channel="Info", condition=True) # Always show commit info
...
def git_pull_changes(repo_path=".", branch=None, verbose=False):
    ...
    write_debug("Pulling latest changes", channel="Info", condition=True) # Always show pull info
...
def git_push_changes(repo_path=".", branch=None, verbose=False):
    ...
    write_debug("Pushing changes", channel="Info", condition=True) # Always show push info
...
def create_local_branch_if_not_exists(repo_path, branch_name, verbose=False):
    ...
    if not branch_exists:
        write_debug(f"Branch '{branch_name}' does not exist locally. Creating...", channel="Info", condition=True) # Always show branch creation info
        run_git_command(["git", "checkout", "-b", branch_name], cwd=repo_path, verbose=verbose)
    else:
        write_debug(f"Branch '{branch_name}' already exists locally.", channel="Info", condition=True) # Always show branch exists info
...
def apply_commit_template(template_name, repo_path, verbose=False):
    ...
    write_debug(f"Using commit template: {template_name}", channel="Info", condition=True) # Always show template info
    write_debug("\nCommit Message Template:\n", channel="BoldMagenta") # Assuming 'BoldMagenta' channel is handled by debug_utils
    write_debug(textwrap.dedent(template), channel="Magenta")      # Assuming 'Magenta' channel is handled by debug_utils
    write_debug("\n---", channel="BoldMagenta")                   # Assuming 'BoldMagenta' channel is handled by debug_utils
    ...
    write_debug(commit_message, channel="Cyan")                     # Assuming 'Cyan' channel is handled by debug_utils
...
def handle_submodules(repo_path, add_pattern, force, branch, submodules_to_process, submodule_add_patterns, submodule_branches, verbose):
    ...
    write_debug("Checking for submodules...", channel="Info", condition=True) # Always show submodule check info
    ...
    for line in submodules_output:
        ...
        if len(parts) > 1:
            ...
            if submodules_to_process != 'all' and submodule_name not in submodules_to_process:
                write_debug(f"Skipping submodule: {submodule_path} as it's not in the process list.", channel="Info")
                continue # Skip if submodule is not in the list to process

            full_submodule_path = os.path.join(repo_path, submodule_path)
            write_debug(f"Entering submodule: {full_submodule_path}", channel="Info", condition=True) # Always show submodule entry
...
def process_git_workflow(add_pattern, force, cwd=".", branch=None, submodules_to_process=None, submodule_add_patterns=None, commit_template=None, submodule_branches=None, create_branch=None, verbose=False):
    ...
    write_debug(f"Processing repository: {repo_path}", channel="Info", condition=True) # Always show repo processing info

    # Check git status
    write_debug("Checking Git Status...", channel="Info", condition=True) # Always show status check info
    if not show_git_status(repo_path, verbose=verbose):
        write_debug("No changes detected. Running git pull...", channel="Success")
    ...
    # Show staged files
    write_debug("Checking Staged Files...", channel="Info", condition=True) # Always show staged files check info
    if not show_staged_files(repo_path, verbose=verbose):
        write_debug("No files were staged. Skipping commit.", channel="Warning")
        write_debug("Proceeding to git pull.", channel="Info")
        git_pull_changes(repo_path, branch=branch, verbose=verbose)
        return
    ...
    if not force:
        ...
        if confirmation == "n":
            ...
            write_debug("Skipping commit, proceeding to git pull.", channel="Info")
            git_pull_changes(repo_path, branch=branch, verbose=verbose)
            return

    # Prompt for commit message
    write_debug("Prompting for Commit Message", channel="Info", condition=True) # Always show commit message prompt info
    if commit_template:
    ...
    if not new_commits:
        write_debug("No new commits to push. Process complete.", channel="Success")
    else:
...
def print_repo_processing_order(args, submodules_output):
    ...
    for i, repo_desc in enumerate(repo_order):
        write_debug(f"{i+1}. {repo_desc}", channel="Info")
