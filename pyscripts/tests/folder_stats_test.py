...
from folder_stats import (
    ...
    gather_dir_totals, _normalize_exts, _normalize_exts_raw, print_hotspots,
    gather_top_files, print_top_files
)
...
def test_top_files_and_exclude_in_tree(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["prog", "R", "-e", "mp4", "-t", "-x", "tmpvids", "-f", "1"])
    main()
    out = capsys.readouterr().out
