...
def rwc_runner(monkeypatch):
    def run(args_list):
        ...
        args = replace_with_clipboard.parser.parse_args(args_list) # Uses replace_with_clipboard.parser
...
def test_replace_file_with_arg(mock_get_clipboard_rwc, tmp_path, capsys, rwc_runner):
    ...
    assert "Replaced contents of" in captured_out
...
def test_replace_creates_new_file_if_not_exists(mock_get_clipboard_rwc, tmp_path, capsys, rwc_runner):
    ...
    assert f"File '{str(test_file)}' does not exist. Creating new file." in captured
    assert "Replaced contents of" in captured
