...
def main(scripts_dir: Path, dotfiles_dir: Path, bin_dir: Path, verbose: bool) -> None:
    ...
    with section("PY SCRIPTS SETUP"):
        ...
        if not aliases and not functions:
            log_warning(f"No definitions found or parsed from '{definitions_file}'. Skipping generation.")
        else:
            log_info(f"Found {len(aliases)} alias and {len(functions)} function definitions to process.")
            ...
            if aliases:
                with section("ALIASES FOR PYTHON SCRIPTS (ZSH/BASH)"):
                    ...
                    try:
                        write_definitions(aliases, bin_dir, alias_config_zsh, str(definitions_file), 'alias', verbose)
                    except Exception as e:
            ...
            if functions:
                with section("SHELL FUNCTIONS (ZSH/BASH)"):
                    ...
                    try:
                        write_definitions(functions, bin_dir, functions_output_file, str(definitions_file), 'func', verbose)
                    except Exception as e:
