...
class ParallelRunner:
    def __init__(
        ...
        self.overall_task = self.progress.add_task("[bold green]Overall Progress[/bold green]", total=len(instance_args))
        self.instance_tasks = {}
        self.parsed_outputs = {}
    ...
    async def run_instance(self, instance_name: str, args: List[str], instance_id: int):
        ...
        task_id = self.progress.add_task(f"[cyan]{instance_name}[/cyan]", total=None)
        self.instance_tasks[instance_id] = task_id

        while retry_count <= self.max_retries:
            if self.paused:
                console.print(f"[bold yellow]Pausing retries for {instance_name}...[/bold yellow]")
            ...
            try:
                ...
                async for line in process.stdout:
                    ...
                    try:
                        parsed = self.parser_func(stdout_buffer, stderr_buffer)
                        if parsed:
                            self.parsed_outputs[instance_id] = parsed
                            self.update_individual_progress(instance_id, parsed)
    ...
    def update_individual_progress(self, instance_id: int, parsed_output: Any):
        ...
        if isinstance(parsed_output, dict) and 'current' in parsed_output and 'total' in parsed_output:
            self.progress.update(task_id, total=parsed_output['total'], completed=parsed_output['current'])
        elif isinstance(parsed_output, (int, float)):
            self.progress.update(task_id, completed=parsed_output)

    async def monitor_overall_progress(self):
        while self.progress.tasks[self.overall_task].completed < self.progress.tasks[self.overall_task].total:
            if self.parsed_outputs:
                try:
                    aggregated = self.aggregate_func(self.parsed_outputs)
                    if aggregated:
                        self.progress.update(self.overall_task, **aggregated)
                except Exception as e:
    ...
    async def run_all(self) -> Dict[int, Any]:
        ...
        with Live(self.progress, refresh_per_second=10):
            tasks = {
                i: asyncio.create_task(self.run_instance(f"Task {i+1}", args, i))
                for i, args in enumerate(self.instance_args)
            }
