...
def test_scenario_multiple_edits_on_same_file(mock_confirm, project):
    config_py = project / "config.py"
    ops = [
        # First, change the line number
        {"file_path": str(config_py), "operation": "replace_block", "locator": {"type": "line_number", "value": "2"}, "content": "class BetterSettings:"},
        # Then, use the *new* content as an anchor
        {"file_path": str(config_py), "operation": "insert_after", "locator": {"type": "block_content", "value": "class BetterSettings:"}, "content": "    # Class-level docstring"},
    ]
...
def test_error_on_malformed_json(capsys):
    with patch('sandr.json_replacer.get_clipboard', return_value="not json ["):
...
def test_error_on_non_array_json(capsys):
    with patch('sandr.json_replacer.get_clipboard', return_value='{"key": "value"}'):
        with pytest.raises(SystemExit):
            json_replacer.main()
    captured = capsys.readouterr()
