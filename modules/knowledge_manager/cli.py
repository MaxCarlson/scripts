...
from . import utils
from . import db # For init_db

# --- Configuration ---
BASE_DATA_DIR: Optional[Path] = None 

# --- Helper Functions for CLI Output ( 그대로 ) ---
...
def print_task(task: Task, include_details: bool = False, base_data_dir: Optional[Path] = None):
    ...
    if task.project_id:
        print(f"Project ID: {task.project_id}") # Could fetch project name
...
def handle_init(args: argparse.Namespace):
    ...
    try:
        ...
        db.init_db(db_path) # This creates tables if they don't exist
        # Also ensure content directories are created by utils.get_base_data_dir and its callers
...
# --- Project Command Handlers ( 그대로, argument names will be updated in parser setup ) ---
...
# --- Task Command Handlers ( 그대로, argument names will be updated in parser setup ) ---
def handle_task_add(args: argparse.Namespace):
    try:
        task = task_ops.create_new_task(
            title=args.title,
            project_identifier=args.project_id, # Changed from args.project
            parent_task_identifier=args.parent_id, # Changed from args.parent
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--data-dir', dest='data_dir', type=Path, default=None, # Explicit dest
        help="Override the default base data directory for this command."
    )
    # No set_defaults for data_dir here, let ops functions use their None default
    # which then triggers utils.get_base_data_dir() default.
    # The global BASE_DATA_DIR will be set in main() if --data-dir is used.
    ...
    init_parser.set_defaults(func=handle_init) # No specific args for init other than global --data-dir
    ...
    task_add_parser.add_argument("-P", "--parent-id", dest="parent_id", type=str, help="Parent task ID for creating a subtask") # Changed from --parent
    task_add_parser.add_argument("-d", "--details", dest="details", type=str, help="Markdown details for the task")
    task_add_parser.add_argument("-r", "--priority", dest="priority", type=int, default=3, choices=range(1,6), help="Priority (1-5, default 3)") # Changed from -P
    task_add_parser.add_argument("-D", "--due-date", dest="due_date", type=str, help="Due date in YYYY-MM-DD format") # Changed from --due
    ...
    task_list_parser.add_argument("-P", "--parent-id", dest="parent_id", type=str, help="Filter tasks by parent task ID") # Changed from --parent
    task_list_parser.add_argument(
        "-a", "--all-subtasks", dest="all_subtasks", action="store_true", # Changed from --all-subs
...
def main(argv: Optional[List[str]] = None):
    ...
    if args.data_dir: # This attribute is guaranteed to exist due to add_argument
        BASE_DATA_DIR = args.data_dir
        # The args.data_dir is now directly passed to ops functions by handlers
    
    if hasattr(args, 'func'):
        try:
            args.func(args) # Pass the full args namespace to handlers
        except Exception as e:
