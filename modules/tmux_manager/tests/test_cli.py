...
class TestCLICommands:
    ...
    @patch('sys.argv', ['tmx', 'closew', '4..10', '-t', 'test_session'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'mvw', '-i', '-1', '-w', '3'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'mvws', '-s', 'ai', '-i', '0'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'swws', '-s', 'ai', '-i', '3'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'mvws', '--from', 'source_sess', '-s', 'target_sess'])
    @patch('tmux_manager.cli.TmuxWindowManager')
...
class TestCLIErrorHandling:
    ...
    @patch('sys.argv', ['tmx', 'closew', '--help'])
...
class TestCLIArgumentParsing:
    ...
    @patch('sys.argv', ['tmx', 'closew', '4..10'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'closew', '1,7,8,11'])
    @patch('tmux_manager.cli.TmuxWindowManager')
...
class TestIntegrationScenarios:
    ...
    @patch('sys.argv', ['tmx', 'mvw', '-i', '0', '-t', 'my_session'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    ...
    @patch('sys.argv', ['tmx', 'closew', '0..5', '-t', 'cleanup_session'])
    @patch('tmux_manager.cli.TmuxWindowManager')
...
class TestSpawnCommand:
    ...
    @patch('sys.argv', ['tmx', 'spawn'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_default(self, mock_manager_class):
        """Test spawn command with defaults."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.spawn_windows.return_value = True
        mock_manager_class.return_value = mock_manager
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=1, panes_per_window=1, target_index=None,
            session_name=None, window_name=None
        )

    @patch('sys.argv', ['tmx', 'spawn', '-c', '3'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_multiple_windows(self, mock_manager_class):
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=3, panes_per_window=1, target_index=None,
            session_name=None, window_name=None
        )

    @patch('sys.argv', ['tmx', 'spawn', '-p', '4'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_with_panes(self, mock_manager_class):
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=1, panes_per_window=4, target_index=None,
            session_name=None, window_name=None
        )

    @patch('sys.argv', ['tmx', 'spawn', '-c', '3', '-p', '2'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_windows_with_panes(self, mock_manager_class):
        """Test spawning multiple windows with panes."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.spawn_windows.return_value = True
        mock_manager_class.return_value = mock_manager
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=3, panes_per_window=2, target_index=None,
            session_name=None, window_name=None
        )

    @patch('sys.argv', ['tmx', 'spawn', '-n', 'workspace'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_with_name(self, mock_manager_class):
        ...
        mock_manager.spawn_windows.assert_called_once_with(
            count=1, panes_per_window=1, target_index=None,
            session_name=None, window_name='workspace'
        )

    @patch('sys.argv', ['tmx', 'spawn', '-i', '0', '-t', 'my_session'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_with_index_and_session(self, mock_manager_class):
        """Test spawning at specific index in specific session."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.spawn_windows.return_value = True
        mock_manager_class.return_value = mock_manager
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=1, panes_per_window=1, target_index=0,
            session_name='my_session', window_name=None
        )

    @patch('sys.argv', ['tmx', 'spawn', '-c', '3', '-p', '2', '-n', 'dev', '-i', '0'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_complex(self, mock_manager_class):
        ...
        assert exc_info.value.code == 0
        mock_manager.spawn_windows.assert_called_once_with(
            count=3, panes_per_window=2, target_index=0,
            session_name=None, window_name='dev'
        )

    @patch('sys.argv', ['tmx', 'spawn'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_spawn_failure(self, mock_manager_class):
        """Test spawn command failure."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.spawn_windows.return_value = False
        mock_manager_class.return_value = mock_manager
...
class TestJumpCommand:
    """Tests for jump command."""

    @patch('sys.argv', ['tmx', 'jump', 'ai'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_jump_with_session(self, mock_manager_class):
        """Test jump command with session name."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.jump_to_session.return_value = True
        mock_manager_class.return_value = mock_manager
        ...
        assert exc_info.value.code == 0
        mock_manager.jump_to_session.assert_called_once_with(session_name='ai')

    @patch('sys.argv', ['tmx', 'jump'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_jump_without_session(self, mock_manager_class):
        """Test jump command without session (fzf selection)."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.jump_to_session.return_value = True
        mock_manager_class.return_value = mock_manager
        ...
        assert exc_info.value.code == 0
        mock_manager.jump_to_session.assert_called_once_with(session_name=None)

    @patch('sys.argv', ['tmx', 'jump', 'nonexistent'])
    @patch('tmux_manager.cli.TmuxWindowManager')
    def test_jump_failure(self, mock_manager_class):
        """Test jump command failure."""
        mock_manager = Mock()
        mock_manager._is_tmux_installed.return_value = True
        mock_manager.jump_to_session.return_value = False
        mock_manager_class.return_value = mock_manager
