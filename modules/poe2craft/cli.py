...
def _state_dir() -> Path:
    """Directory for persisted state (timestamps, user prefs)."""
...
def _stdout_json(obj: Any) -> None:
    """Prints only JSON to stdout (tests rely on clean stdout)."""
...
def cmd_currencies(args: argparse.Namespace) -> None:
    ...
    with progress("Currencies", total=1, quiet=(args.format == "json")) as bar:
        data = [c.__dict__ for c in client.fetch_stackable_currency()]
...
def cmd_base_items(args: argparse.Namespace) -> None:
    ...
    if args.save:
        # Save combined list to --save path
...
def cmd_prices(args: argparse.Namespace) -> None:
    ...
    # Optional auto-refresh when stale
    if args.if_stale:
    ...
    # For stdout, tests expect a plain mapping only
    if args.format == "json":
        if args.output:
...
def cmd_consumables(args: argparse.Namespace) -> None:
    """Download currencies + omens + essences. If -O/--output-dir is set, write 3 separate files there."""
    ...
    with progress("Consumables", total=3, quiet=False) as bar:
        cur = [c.__dict__ for c in client.fetch_stackable_currency()]
...
def cmd_all(args: argparse.Namespace) -> None:
    """Download currencies, omens, essences, base items (curated/all), and prices. Writes separate files when -O is used."""
    out_dir = Path(args.output_dir).expanduser() if args.output_dir else None
    if out_dir:
        out_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Save per-file outputs per your requested names
    if out_dir:
        _dump_json_to(out_dir / "currencies.json", cur)
        _dump_json_to(out_dir / "omens.json", om)
        _dump_json_to(out_dir / "essences.json", es)
    ...
    # Also print a summary (JSON) to stdout or file if requested
    summary = {
        "saved": bool(out_dir),
        "league": league,
        "counts": {
