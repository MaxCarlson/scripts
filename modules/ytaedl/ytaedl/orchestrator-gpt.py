...
- --skip-scan mode now assigns URL files to workers RANDOMLY with exclusivity
  (no two workers get the same file at the same time). Subsequent acquisitions
  remain random as long as there is remaining work.
- -a/--num-aebn-dl and -y/--num-ytdl-dl preserved; if threads < (a+y), we scale
  the worker counts by ratio and cap at --threads.
"""
...
class _Coordinator:
    ...
    def _candidates(self) -> List["_WorkFile"]:
        # Build list of unassigned items with remaining > 0
        items: List[_WorkFile] = []
        for w in self._work.values():
            key = str(w.url_file.resolve())
            if self._assigned.get(key, 0) == 0 and w.remaining > 0:
                items.append(w)
        return items
    ...
    def acquire_next(self) -> Optional["_WorkFile"]:
        with self._lock:
            ...
            if self._mode == "largest":
                cand.sort(key=lambda w: (-w.remaining, w.stem))
            else:
                random.shuffle(cand)
            w = cand[0]
            key = str(w.url_file.resolve())
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    with ui:
        ...
        if args.skip_scan:
            ...
            random.shuffle(all_work)  # seed some randomness for initial pool shape
        else:
        ...
        num_a = max(0, int(args.num_aebn_dl))
        num_y = max(0, int(args.num_ytdl_dl))
        total_req = num_a + num_y
        total_allowed = max(0, int(args.threads))
        ...
        # Selection mode: random only when skip-scan is active; else prefer largest remaining first.
