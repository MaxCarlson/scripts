...
def _ui_set_worker_scan_progress(
    ui, slot: int, set_name: str, i: int, total: int, eta_s: Optional[float]
) -> None:
    try:
...
def _scan_one_file_main_with_ytdlp(
    url_file: Path, out_root: Path, *, yt_dlp: str, template: str,
    pause_evt: threading.Event, stop_evt: threading.Event, ui: object, slot: int, log_fn
) -> URLFileInfo:
    urls = read_urls_from_files([url_file])
    total = len(urls)
    stem = url_file.stem
    dest = _infer_dest_dir(out_root, url_file)
    seen, already, bad = 0, 0, 0
    start = time.time()
    _ui_set_worker_scan_progress(ui, slot, stem, 0, total, None)

    for url in urls:
        if stop_evt.is_set(): break
        while pause_evt.is_set() and not stop_evt.is_set(): time.sleep(0.05)
        rc, value = _ytdlp_get_expected_filename(yt_dlp, url, template)
        exists_flag: Optional[bool]
        if rc == 0:
            exists_flag = _exists_with_dup(dest, value)
            if exists_flag: already += 1
        else:
            exists_flag = None
            bad += 1
    ...
    return URLFileInfo(
        url_file=url_file, stem=stem, source="main", out_dir=dest, url_count=total,
        downloaded=already, bad=bad, remaining=remaining, viable_checked=True
    )
...
def _build_worklist(snap: CountsSnapshot) -> List[_WorkFile]:
    ...
    for key, rec in snap.files.items():
        url_file = Path(key)
        try: urls = read_urls_from_files([url_file])
        ...
        work.append(
            _WorkFile(
                url_file=url_file, stem=str(rec.get("stem") or url_file.stem),
                source=str(rec.get("source") or "main"), out_dir=out_dir,
                urls=urls, remaining=remaining
            )
        )
    return work

def _build_worklist_from_disk(
    main_url_dir: Path, ae_url_dir: Path, out_root: Path, single_files: Optional[List[Path]] = None
) -> List[_WorkFile]:
    work: List[_WorkFile] = []
    seen_files = set()
    ...
    def _process_file(url_file: Path, source: str):
        try:
            ...
            if resolved_path in seen_files: return
            urls = read_urls_from_files([url_file])
            out_dir = _infer_dest_dir(out_root, url_file)
            work.append(
                _WorkFile(
                    url_file=url_file, stem=url_file.stem, source=source,
                    out_dir=out_dir, urls=urls, remaining=len(urls)
                )
            )
        ...
        def _process_dir(d: Path, source: str):
            if d.is_dir():
                for url_file in sorted(d.glob("*.txt")):
                    if url_file.is_file(): _process_file(url_file, source)
...
def _scan_all_parallel(
    main_url_dir: Path, ae_url_dir: Path, out_root: Path, single_files: Optional[List[Path]], *,
    yt_dlp: str, ytdlp_template: str, n_workers: int, counts_path: Path,
    ui, log_fn, stop_evt: threading.Event, pause_evt: threading.Event
) -> CountsSnapshot:
    all_files_map: Dict[str, Tuple[Path, str]] = {}
    if not single_files:
        for p in sorted(main_url_dir.glob("*.txt")):
            if p.is_file(): all_files_map[str(p.resolve())] = (p, "main")
        for p in sorted(ae_url_dir.glob("*.txt")):
            if p.is_file(): all_files_map[str(p.resolve())] = (p, "ae")
    else:
        resolved_ae_dir = str(ae_url_dir.resolve())
        for f in single_files:
            if f.is_file():
    ...
    try: ui.set_scan_log_path(counts_path.with_name("ytaedl-scan-results.tsv"))
    except Exception: pass

    q: "queue.Queue[tuple[int, Path, str]]" = queue.Queue()
    for idx, (f, src) in enumerate(all_files): q.put((idx, f, src))
    files_lock = threading.Lock()

    def worker(slot: int):
        while not stop_evt.is_set():
            try: _idx, f, src = q.get_nowait()
            except queue.Empty: break
            if src == "ae":
                ui.set_scan_slot(slot, f"ae:{f.stem} (skipped)"); ui.advance_scan(1); continue
    ...
    threads = [threading.Thread(target=worker, args=(i,), daemon=True, name=f"scan-{i+1}") for i in range(max(1, n_workers))]
    for t in threads: t.start()
    while any(t.is_alive() for t in threads):
        if stop_evt.is_set(): break
        ui.pump(); time.sleep(0.05)
...
class _Coordinator:
    ...
    def acquire_next(self) -> Optional["_WorkFile"]:
        with self._lock:
            ...
            if self._mode == "random": random.shuffle(candidates)
            else: candidates.sort(key=lambda w: (-w.remaining, w.stem))
    ...
    def release(self, wf: "_WorkFile", *, remaining_delta: int = 0) -> None:
        with self._lock:
            ...
            if key not in self._work: return
            cur = self._work[key]
            new_remaining = max(0, int(cur.remaining) + int(remaining_delta))
            self._work[key] = dataclasses.replace(cur, remaining=new_remaining)
            if key in self._assigned: self._assigned[key] = max(0, self._assigned[key] - 1)

# ---------------------------------------------------------------------------

def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="ytaedl-orchestrate", description="Orchestrate scanning and downloads.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def main(argv: Optional[List[str]] = None) -> int:
    args = parse_args(argv); random.seed()
    args.url_dir, args.ae_url_dir, args.output_dir, args.counts_file = Path(args.url_dir), Path(args.ae_url_dir), Path(args.output_dir), Path(args.counts_file)
    log_fp = None
    if args.log_file:
        try: args.log_file.parent.mkdir(parents=True, exist_ok=True); log_fp = args.log_file.open("a", encoding="utf-8")
        except Exception: pass
    def olog(msg: str):
        if log_fp:
            try: log_fp.write(f"[{_now_str()}] {msg}\n"); log_fp.flush()
            except Exception: pass
    try: ui = TermdashUI(num_workers=max(1, args.threads), total_urls=0) if not args.no_ui else SimpleUI(num_workers=max(1, args.threads), total_urls=0)
    except Exception as ex: olog(f"UI fallback: {ex}"); ui = SimpleUI(num_workers=max(1, args.threads), total_urls=0)
    ...
    def _key_reader():
        ...
        if os.name == "nt":
            ...
            while not stop_evt.is_set():
                if msvcrt.kbhit():
                    ch = msvcrt.getwch()
                    if ch in ("z", "Z"): pause_evt.set() if not pause_evt.is_set() else pause_evt.clear(); ui.set_paused(pause_evt.is_set())
    ...
    with ui:
        olog("orchestrator start"); threading.Thread(target=_key_reader, daemon=True, name="keys").start()
        ...
        if args.skip_scan:
            ...
            all_work = _build_worklist_from_disk(args.url_dir, args.ae_url_dir, args.output_dir, single_files=single_files_provided)
        else:
        ...
        archive_path = args.archive; archived_urls = load_archive(archive_path) if archive_path else set(); archive_lock = threading.Lock()
        aebn_work, ytdlp_work = [w for w in all_work if w.source == 'ae'], [w for w in all_work if w.source == 'main']
        ...
        else: aebn_threads, ytdlp_threads = num_a, num_y
        ...
        aebn_coord, ytdlp_coord = _Coordinator(aebn_work, mode=sel_mode), _Coordinator(ytdlp_work, mode=sel_mode)
        cfg = _build_config(args); object.__setattr__(cfg, "_orchestrator_max_dl", int(args.max_dl))
        ...
        def worker(slot: int, coordinator: _Coordinator):
            while not stop_evt.is_set():
                ...
                successful_downloads, cap = 0, getattr(cfg, "_orchestrator_max_dl", 3)
                for idx, url in enumerate(wf.urls, start=1):
                    if successful_downloads >= cap or stop_evt.is_set(): break
                    ...
                    item = DownloadItem(id=get_next_item_id(), url=url, output_dir=wf.out_dir, source=URLSource(file=wf.url_file, line_number=idx, original_url=url))
                    setattr(item, 'total_in_set', len(wf.urls))
                    downloader, url_was_successful = get_downloader(url, cfg), False
                    try:
                        for ev in downloader.download(item):
                            ...
                            if isinstance(ev, FinishEvent):
                                ...
                                if result.status in (DownloadStatus.COMPLETED, DownloadStatus.ALREADY_EXISTS):
                                    ...
                                    if archive_path:
                                        with archive_lock:
                                            if url not in archived_urls: write_to_archive(archive_path, url); archived_urls.add(url)
                            ui.pump()
                    ...
                    if url_was_successful: successful_downloads += 1
                coordinator.release(wf, remaining_delta=-(successful_downloads))
                olog(f"Worker {slot}: Released '{wf.url_file.name}'. Completed {successful_downloads} downloads.")
            olog(f"Worker {slot}: No more work. Shutting down.")

        threads = []
        for i in range(aebn_threads): threads.append(threading.Thread(target=worker, args=(i, aebn_coord), daemon=True, name=f"dl-ae-{i+1}"))
        for i in range(ytdlp_threads): threads.append(threading.Thread(target=worker, args=(aebn_threads + i, ytdlp_coord), daemon=True, name=f"dl-yt-{i+1}"))
        for t in threads: t.start()
        try:
            while any(t.is_alive() for t in threads) and not stop_evt.is_set(): ui.pump(); time.sleep(0.1)
        finally:
            for t in threads: t.join(timeout=1.0)
