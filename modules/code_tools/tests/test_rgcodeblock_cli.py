...
def mock_file_content(monkeypatch):
    ...
    def side_effect_open(filename, mode='r', *args, **kwargs):
        ...
        if filename_str in files_dict and 'r' in mode:
            ...
            # Ensure __enter__ returns the handle for 'with open(...)' context
...
# Helper to run main for the CLI script (Corrected)
def run_cli_main_with_args(args_list, capsys):
    ...
    # No longer need the global args_for_main_thread workaround
...
def test_list_languages_cli(capsys):
    ...
    # Provide dummy pattern/path args required by argparse before --list-languages check
    out, err, code = run_cli_main_with_args(["dummy_pattern", "--list-languages"], capsys) # <<< ADDED dummy pattern
    assert code == 0, f"Expected exit 0, got {code}. Err: {err}" # <<< CORRECTED ASSERTION
...
def test_basic_match_and_extraction_text_format_cli(mock_rg_subprocess, mock_file_content, capsys):
    ...
    expected_block = PYTHON_LINES_1[2:5] # __init__ method block lines
    ...
    assert code == 0
    assert "╠════" in out # Check for part of fancy separator <<< CORRECTED ASSERTION
    assert "File: test.py:4" in out
    assert "Highlight(s) (1): \"__init__\"" in out
    assert re.search(r"def .*\033\[1;31m__init__\033\[0m.*:", out)
    assert "self.value = value" in out
...
def test_json_format_output_cli(mock_rg_subprocess, mock_file_content, capsys):
    ...
    expected_block = JSON_LINES_1 # Whole file block
    ...
    try:
        ...
        # Check if SUBSTRING is in ANY of the lines in the block list <<< CORRECTED ASSERTION
    ...
    except AssertionError as ae: pytest.fail(f"Assertion Error: {ae}\nOutput:\n{out}")
...
def test_cli_stats_output_format(mock_rg_subprocess, mock_file_content, capsys):
    ...
    assert "Unique Code Blocks Processed: 2" in out
...
def test_cli_ruby_extraction(mock_rg_subprocess, mock_file_content, capsys):
    ...
    output_lines = [l for l in out.split('Match')[-1].splitlines() if l.strip()] # Split on generic Match
    assert output_lines[-1].strip().startswith("end") # More robust check for end
...
def test_cli_lua_extraction(mock_rg_subprocess, mock_file_content, capsys):
    ...
    output_lines = [l for l in out.split('Match')[-1].splitlines() if l.strip()]
    assert output_lines[-1].strip() == "end" # Lua end is usually exactly 'end'
...
def test_cli_include_ext_args(mock_rg_subprocess, capsys):
    ...
    assert "rgcbinclude0:*.py" in cmd
    assert "-t" in cmd
    assert "rgcbinclude0" in cmd
...
def test_cli_max_block_lines_truncation(mock_rg_subprocess, mock_file_content, capsys):
    ...
    assert "l2" in out
    assert re.search(r"\.\.\. \(3 lines truncated\) \.\.\.", out) # <<< CORRECTED ASSERTION (check ellipsis text)
    assert "l6" in out
...
def test_cli_separator_styles(mock_rg_subprocess, mock_file_content, capsys):
    ...
    with patch.object(rgc_lib, 'get_language_type_from_filename', return_value=("unknown", "txt")), \
         patch.object(rgc_lib, 'EXTRACTOR_DISPATCH_MAP', {}):
